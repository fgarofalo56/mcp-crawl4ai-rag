...
# Configure logging to stderr for stdio protocol compatibility
...
class AIHallucinationDetector:
    ...
    def _print_batch_summary(self, results: list[dict]):
        ...
        if not results:
            print("No scripts were successfully processed.", file=sys.stderr, flush=True)
            return

        print("\n" + "=" * 80, file=sys.stderr, flush=True)
        print("🚀 BATCH HALLUCINATION DETECTION SUMMARY", file=sys.stderr, flush=True)
        ...
        avg_confidence = (
            sum(r["validation_summary"]["overall_confidence"] for r in results) / total_scripts
        )

        print(f"Scripts Processed: {total_scripts}", file=sys.stderr, flush=True)
        print(f"Total Validations: {total_validations}", file=sys.stderr, flush=True)
        print(f"Average Confidence: {avg_confidence:.1%}", file=sys.stderr, flush=True)
        print(f"Total Hallucinations: {total_hallucinations}", file=sys.stderr, flush=True)

        print("\nAggregated Results:", file=sys.stderr, flush=True)
        print(
        ...
        print("\n🚨 Scripts with Most Hallucinations:", file=sys.stderr, flush=True)
...
Examples:
  ...
  python ai_hallucination_detector.py script.py --no-markdown
    ...
    parser.add_argument(
        "--neo4j-uri", default=None, help="Neo4j URI (default: from environment NEO4J_URI)"
    )

    parser.add_argument(
        "--neo4j-user", default=None, help="Neo4j username (default: from environment NEO4J_USER)"
    )
