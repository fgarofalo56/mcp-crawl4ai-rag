...
class EnvironmentManager:
    ...
    def validate_environment(self, raise_on_error: bool = True) -> tuple[bool, dict]:
        ...
        if results["valid"]:
            logger.info(
                f"✓ All {len(results['present_required'])} required environment variables are set"
            )
        else:
            logger.error(
    ...
    def get_env_bool(self, var_name: str, default: bool = False) -> bool:
        ...
        if value_str in ("true", "yes", "1", "on"):
            ...
            logger.warning(
                f"Invalid boolean value for '{var_name}': {value_str}. " f"Using default: {default}"
            )
            return default
...
def get_env_int(var_name: str, default: int | None = None, **kwargs) -> int:
    """Get environment variable as int. See EnvironmentManager.get_env_int()."""
    return env_manager.get_env_int(var_name, default, **kwargs)


def get_env_float(var_name: str, default: float | None = None, **kwargs) -> float:
