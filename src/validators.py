...
from .error_handlers import ValidationError, validate_file_path, validate_range, validate_url
...
class InputValidator:
    ...
    def validate_chunk_size(chunk_size: int) -> int:
        ...
        """
        is_valid, error = validate_range(
            chunk_size, min_val=100, max_val=50000, field_name="chunk_size"
        )
    ...
    def validate_concurrent_limit(limit: int) -> int:
        ...
        """
        is_valid, error = validate_range(limit, min_val=1, max_val=50, field_name="max_concurrent")
    ...
    def validate_match_count(count: int) -> int:
        ...
        """
        is_valid, error = validate_range(count, min_val=1, max_val=100, field_name="match_count")
    ...
    @staticmethod
    def validate_source_filter(source: str | None) -> str | None:
        """
    ...
    def validate_command(command: str, allowed_commands: list[str]) -> str:
        ...
        if command not in allowed_commands:
            raise ValidationError(
                f"Invalid command: {command}. " f"Allowed commands: {', '.join(allowed_commands)}"
            )
