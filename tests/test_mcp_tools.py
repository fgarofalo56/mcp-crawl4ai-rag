...
class TestCrawlSinglePage:
    ...
    async def test_crawl_single_page_success(
        self, mock_context, mock_crawler, mock_supabase_client, mock_env_vars
    ):
        """Test successful single page crawl."""
        from src.tools.crawling_tools import crawl_single_page
    ...
    @pytest.mark.asyncio
    async def test_crawl_single_page_with_code_extraction(
        ...
        with patch("src.utils.add_documents_to_supabase"):
            with patch("src.utils.add_code_examples_to_supabase"):
                with patch("src.utils.update_source_info"):
                    with patch("src.utils.extract_source_summary", return_value="Summary"):
                        with patch(
                            "src.utils.extract_code_blocks",
                            return_value=[{"code": "def test(): pass"}],
                        ):
                            with patch(
                                "src.utils.generate_code_example_summary",
                                return_value="Test function",
                            ):
                                response = await crawl_single_page(
...
class TestSmartCrawlUrl:
    ...
    @pytest.mark.asyncio
    async def test_smart_crawl_sitemap(
        self, mock_context, mock_crawler, mock_supabase_client, mock_env_vars
    ):
        """Test smart crawl with sitemap URL."""
        from src.tools.crawling_tools import smart_crawl_url

        mock_context.request_context.lifespan_context.crawler = mock_crawler
        mock_context.request_context.lifespan_context.supabase_client = mock_supabase_client

        with (
            patch("src.crawling_utils.is_sitemap", return_value=True),
            patch("src.crawling_utils.parse_sitemap", return_value=["https://example.com/page1"]),
            patch(
                "src.crawl_helpers.crawl_batch",
                return_value=[{"url": "https://example.com/page1", "markdown": "Content"}],
            ),
    ...
    @pytest.mark.asyncio
    async def test_smart_crawl_txt_file(
        self, mock_context, mock_crawler, mock_supabase_client, mock_env_vars
    ):
        ...
        with (
            patch("src.crawling_utils.is_txt", return_value=True),
            patch(
                "src.crawl_helpers.crawl_markdown_file",
                return_value=[{"url": "https://example.com/llms.txt", "markdown": "Content"}],
            ),
            patch("src.utils.add_documents_to_supabase"),
        ):
            with patch("src.utils.update_source_info"):
    ...
    @pytest.mark.asyncio
    async def test_smart_crawl_webpage(
        self, mock_context, mock_crawler, mock_supabase_client, mock_env_vars
    ):
        ...
        with patch("src.crawling_utils.is_sitemap", return_value=False):
            with patch("src.crawling_utils.is_txt", return_value=False):
                with patch(
                    "src.crawl_helpers.crawl_recursive_internal_links",
...
class TestCrawlWithStealthMode:
    ...
    async def test_stealth_crawl_success(self, mock_context, mock_supabase_client, mock_env_vars):
        ...
        from src.tools.crawling_tools import crawl_with_stealth_mode
            ...
            with patch("src.crawling_utils.is_txt", return_value=False):
                with patch("src.crawling_utils.is_sitemap", return_value=False):
                    with patch(
                        ...
                        with patch("src.utils.add_documents_to_supabase"):
                            response = await crawl_with_stealth_mode(
                                mock_context, "https://example.com"
                            )
...
class TestCrawlWithMultiUrlConfig:
    ...
    async def test_multi_url_crawl(
        ...
        from src.tools.crawling_tools import crawl_with_multi_url_config
...
class TestCrawlWithMemoryMonitoring:
    ...
    @pytest.mark.asyncio
    async def test_memory_monitored_crawl(
        self, mock_context, mock_crawler, mock_supabase_client, mock_env_vars
    ):
        """Test memory-monitored crawl."""
        from src.tools.crawling_tools import crawl_with_memory_monitoring
            ...
            with patch("src.crawling_utils.is_txt", return_value=False):
                with patch("src.crawling_utils.is_sitemap", return_value=False):
                    with patch(
                        ...
                        with patch("src.utils.add_documents_to_supabase"):
                            response = await crawl_with_memory_monitoring(
                                mock_context, "https://example.com"
                            )
...
class TestPerformRagQuery:
    ...
    @pytest.mark.asyncio
    async def test_rag_query_vector_search(
        self, mock_context, mock_supabase_client, mock_env_vars, sample_search_results
    ):
        ...
        with patch("src.utils.search_documents", return_value=sample_search_results):
    ...
    @pytest.mark.asyncio
    async def test_rag_query_hybrid_search(
        self, mock_context, mock_supabase_client, mock_env_vars, monkeypatch, sample_search_results
    ):
...
class TestSearchCodeExamples:
    ...
    @pytest.mark.asyncio
    async def test_search_code_examples_enabled(
...
class TestParseGithubRepository:
    ...
    async def test_parse_repo_disabled(self, mock_context, mock_env_vars):
        ...
        from src.tools.knowledge_graph_tools import parse_github_repository
    ...
    async def test_parse_repo_success(
        self, mock_context, mock_repo_extractor, mock_env_vars, monkeypatch
    ):
        ...
        mock_repo_extractor.driver.session.return_value.__aenter__ = AsyncMock(
            return_value=mock_session
        )
        mock_repo_extractor.driver.session.return_value.__aexit__ = AsyncMock()

        response = await parse_github_repository(
            mock_context, "https://github.com/user/test-repo.git"
        )
...
class TestCheckAiScriptHallucinations:
    ...
    async def test_hallucination_check_disabled(self, mock_context, mock_env_vars):
        ...
        from src.tools.knowledge_graph_tools import check_ai_script_hallucinations
    ...
    @pytest.mark.asyncio
    async def test_hallucination_check_success(
        self, mock_context, mock_knowledge_validator, mock_env_vars, monkeypatch, tmp_path
    ):
        ...
        with patch("knowledge_graphs.ai_script_analyzer.AIScriptAnalyzer") as mock_analyzer_class:
            with patch(
                "knowledge_graphs.hallucination_reporter.HallucinationReporter"
...
class TestQueryKnowledgeGraph:
    ...
    async def test_query_kg_disabled(self, mock_context, mock_env_vars):
        ...
        from src.tools.knowledge_graph_tools import query_knowledge_graph
    ...
    @pytest.mark.asyncio
    async def test_query_kg_repos_command(
        self, mock_context, mock_repo_extractor, mock_env_vars, monkeypatch
    ):
    ...
    @pytest.mark.asyncio
    async def test_query_kg_invalid_command(
        self, mock_context, mock_repo_extractor, mock_env_vars, monkeypatch
    ):
...
class TestHelperFunctions:
    ...
    def test_is_sitemap_true(self):
        ...
        from src.crawling_utils import is_sitemap
    ...
    def test_is_txt_true(self):
        ...
        from src.crawling_utils import is_txt
    ...
    def test_extract_section_info(self):
        ...
        from src.crawling_utils import extract_section_info
    ...
    def test_rerank_results(self):
        ...
        results = [{"content": "text 1"}, {"content": "text 2"}, {"content": "text 3"}]
    ...
    def test_validate_neo4j_connection(self, mock_env_vars):
        ...
        from src.validators import validate_neo4j_connection
    ...
    def test_validate_script_path_valid(self, tmp_path):
        ...
        from src.validators import validate_script_path
    ...
    def test_validate_github_url_valid(self):
        ...
        from src.validators import validate_github_url
