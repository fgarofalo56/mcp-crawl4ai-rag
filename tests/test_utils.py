...
class TestDocumentOperations:
    ...
    def test_add_documents_to_supabase_basic(self, mock_supabase_client, mock_env_vars):
        ...
        with patch('src.utils.create_embeddings_batch', return_value=[[0.1] * 1536]):
            ...
            assert mock_supabase_client.table().delete().in_().execute.called or \
                   mock_supabase_client.table().insert().execute.called
    
    def test_add_documents_with_contextual_embeddings(self, mock_supabase_client, mock_env_vars, monkeypatch):
        """Test adding documents with contextual embeddings enabled."""
        monkeypatch.setenv("USE_CONTEXTUAL_EMBEDDINGS", "true")
        
        with patch('src.utils.create_embeddings_batch', return_value=[[0.1] * 1536]):
            with patch('src.utils.generate_contextual_embedding', return_value=("contextual text", True)):
                add_documents_to_supabase(
                    mock_supabase_client,
                    urls=["https://example.com"],
                    chunk_numbers=[0],
    ...
    def test_search_documents_success(self, mock_supabase_client, mock_env_vars, sample_search_results):
    ...
    def test_extract_source_summary_success(self, mock_openai_client, mock_env_vars):
        ...
        with patch('src.utils.client', mock_openai_client):
            summary = extract_source_summary(
                source_id="example.com",
                content="This is documentation for the example library."
            )
